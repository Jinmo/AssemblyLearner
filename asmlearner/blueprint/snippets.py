...
def snippet_form(snippet_id=None):
    if snippet_id is not None:
        snippet = g.db.query('SELECT s.id, s.filename, s.code FROM snippets AS s WHERE id=? AND owner=?',
                             (snippet_id, session['user']['id']), True)
    else:
        snippet = None
...
def snippet_upload(snippet_id=None):
    ...
    if snippet_id is not None:
        try:
            g.db.query('UPDATE snippets SET filename=?, code=? WHERE id=? AND owner=?',
                       (filename, code, snippet_id, owner))
            g.db.commit()
