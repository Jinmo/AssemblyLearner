...
void dump_regs(struct user_regs_struct *_user) {
    printf("register dump at %08x\n", (unsigned int)_user->eip);
    printf(" EAX=%08x EBX=%08x\n", (unsigned int)_user->eax, (unsigned int)_user->ebx);
    printf(" ECX=%08x EDX=%08x\n", (unsigned int)_user->ecx, (unsigned int)_user->edx);
    printf(" EDI=%08x ESI=%08x\n", (unsigned int)_user->edi, (unsigned int)_user->esi);
    printf(" EBP=%08x ESP=%08x\n", (unsigned int)_user->ebp, (unsigned int)_user->esp);
    puts("");
}
...
int main(int argc, char **argv) {
    ...
    time_limit.rlim_cur = time_limit.rlim_max = 1;
    proc_limit.rlim_cur = proc_limit.rlim_max = 1;

    alarm(1); // execution time limit
    setrlimit(RLIMIT_CPU, &time_limit); // cpu time limit
    ...
    pid = fork();
    setrlimit(RLIMIT_NPROC, &proc_limit); // process limit
    if(pid == 0) {
        // ptrace init
